...
function Vector3(x = 0, y = x, z = x) : Vector2(x, y) constructor {
...
function Vector4(x = 0, y = x, z = x, w = x) : Vector3(x, y, z) constructor{
	...
	#region Basic functionalities of Vector3
		...
		// If this function is only called with one argument, every component
		// of the vector will be set to that value.
		static Set = function(x, y = x, z = x, w = x) {
			self.x = x;
			self.y = y;
			self.z = z;
			self.w = w;
