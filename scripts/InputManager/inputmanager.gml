...
function InputManager() : Component() constructor {
    ...
    #region PRIVATE ATTRIBUTES AND METHODS TO MANAGE THE STRUCT
        ...
        /*
        static Draw = function(config, instancesDir, keysDir, func) {
        ...
        */
        ...
        static toString = function(tab="") {
        	...
        	for(var i=0;i<array_length(configurations);i++) {
        		str += tab + configurations[i].toString(tab + TABSTR) + "\n";
        	}
        	str += tab + "]";
    ...
    #region MODIFIERS
        ...
        static SetConfiguration = function(config) {
        	var newConf = GetConfiguration(config);
        	if ( newConf == -1 ) currentConfig = config;	
        };
...
function InputConfiguration(name) constructor {
	...
	static Check = function(input, type) {
		...
		if (Console.visible && name!="General") return -1;
	...
	static toString = function(tab="") {
		...
		for( var i = 0 ; i < array_length(self.instances) ; i++ ) {
			str += tab + self.instances[i].toString(tab + TABSTR) + "\n";
		}
		str += tab + "]"
		return str;
	};
...
function InputInstance(name, long_press_time, double_tap_time, repeated_time) constructor {
	...
	static Draw = function(func, i, drawInstancesSelector, drawKeysSelector) {
		...
		for( var j=0 ; j<array_length(self.keys) ; j++ ) {
			...
			func(self.name, KeyToString(self.keys[j].key), i, j, selected);
